...
while numrows_raw_data > 0:
    ...
    numrows = c.execute("select classification_id, annotations from raw_data where reference_key= %s",(numrows_raw_data_str,))
    ...
    #    var1_x = var1[:var1.find(b"\"\"}")+2]
        ...
        c.execute("INSERT INTO click_data (classification_id, x_coord, y_coord, label, reference_key) VALUES (%s, %s, %s, %s, NULL)",(rows1[0], var1_1, var1_2, var1_3))
        conn.commit()
